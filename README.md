...
- `frontend/Dockerfile` — Node 20 빌드 → nginx 서빙
- `docker-compose.yml` — backend:8000, frontend:80, `.env` 주입, `data/jobs/` volume mount
- `nginx.conf` — `Accept: text/html`(브라우저 직접 접근)이면 React 앱 서빙, fetch는 backend로 reverse proxy

---

### CI/CD

`.github/workflows/ci.yml` — `main` 브랜치 push 시 자동 실행됩니다.

```
테스트 → 백엔드 이미지 빌드 및 Docker Hub 푸시 → 프론트엔드 이미지 빌드 및 Docker Hub 푸시
```
...
빌드된 이미지:
- `{DOCKERHUB_USERNAME}/ideavault-backend:latest`
- `{DOCKERHUB_USERNAME}/ideavault-frontend:latest`
...
├── .github/
│   └── workflows/
│       └── ci.yml               # main push → 테스트 → Docker Hub 이미지 빌드·푸시
├── backend/
...
│   ├── Dockerfile
│   ├── tests/
│   │   └── test_health.py       # GET /health, GET /ready 테스트
│   ├── routers/
...
│   └── agent_logs/              # CLI 실행 기록 (python -m agents.run)
│       ├── generated_prds/      # CLI 실행으로 생성된 PRD (prd_{job_id}.md)
│       └── run_{job_id}.log     # 에이전트별 I/O 블록 로그
├── docs/
│   ├── ai-usage-log.md          # AI 협업 기록 (단계별 작업 내용·프롬프트·발견된 문제)
│   ├── coding_prompts/          # Claude Code에 실제로 사용한 프롬프트
│   └── backend_logs/            # 백엔드 구현 단계별 설계 문서
