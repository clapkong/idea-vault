...
**검수 방식**
`run.log`로 에이전트별 I/O 직접 분석 → 프롬프트 수정 → 코드 재반영 반복. 실행해보기 전 토큰 낭비를 최소화하기 위해 AI가 생성한 프롬프트 일부만 발췌 검토하고, 최종본은 직접 작업한 파일로만 에이전트를 실행하였다.

**AI가 만든 코드 중 수정이 필요했던 부분**
- 특정 문구를 과도하게 강력하게 하거나 말 없이 생략하는 경우가 존재
- 원하는 문구를 의도에 맞지 않게 변경하는 경우 존재
- 코드 구현 단계에서 리팩토링 시 기존 코드를 최대한 보존하려는 경향 — 새 방식으로 전환되어 기존 코드가 불필요해진 경우에도 덧대는 방식을 고수해 코드베이스가 복잡해지는 경우 발생. 실행 전 직접 검토 후 불필요한 코드 제거를 명시적으로 요청하는 방식으로 해소.
...
- 프롬프트 및 구현 기획을 Claude 세션과 대화하며 사전에 구체적으로 작성 (`docs/ralph_mode/ralph_prompt.md`)
...
**주요 장점 및 관찰 사항**
- 디자인이 구상한 것과 거의 99% 일치
- FastAPI 엔드포인트 연결이 원활히 처리됨
- 프롬프트에 명시한 대로 매 루프마다 git commit 자동 실행
- 루프를 거치며 기능이 점진적으로 발전하는 구조 확인
...
**프롬프트** (Claude Code)
> `index.css`는 CSS 변수 + html/body/root 리셋만 담당하도록 하고, `.btn-primary` 공통 버튼 스타일은 `App.css`로 옮겨줘. 그리고 모든 CSS 파일에 주석을 달아줘. 파일 상단에 역할 한 줄, 셀렉터마다 한 줄, 비직관적인 속성은 파일 내 첫 등장 시에만 인라인 설명.
...
**프롬프트** (Claude Code)
> JSX 파일에 주석을 달아줘. 규칙: (1) 파일 상단 — 컴포넌트 역할 한 줄 (2) 상수/설정 객체 — 각각 한 줄 (3) 함수/컴포넌트 선언 전 — 한 줄 (4) 훅(useState, useRef, useEffect, useMemo) — 각각 한 줄 (5) JSX 내 주요 블록 — 한 줄 (6) 비직관적 패턴 — 파일 내 첫 등장만, 이후 동일 파일·다른 파일 재등장 생략 (7) 파일 내 일관성 유지. 말투는 "~한다" 서술형 금지, 명사구/라벨 스타일로 간결하게.
